...
namespace WebUI
{
    public static class SocketServer
    {
        ...
        public static Task Listener(int port)
        {
                ...
                try
                {
                    using (socket)
                    {
                        ...
                        do
                        {
                            ...
                            Console.WriteLine($"client string is:{fromClient}");
                            WriteLine($"read {read} bytes: {fromClient}");
                            if (string.Compare(fromClient, "shutdown", ignoreCase: true) == 0)
                            {
                                completed = true;
                            }
                            if (string.Compare(fromClient.Trim(), "time", ignoreCase: true) == 0)
                            {
                                fromClient = DateTime.Now.ToString();
