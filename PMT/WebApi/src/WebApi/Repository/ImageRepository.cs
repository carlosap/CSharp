    ...
    public class ImageRepository: IImage
    {
        public async Task<List<DictionaryItem>>Get(string keyword, string language)
        {
            return await Task.Run(async () =>
            {
                ...
                try
                {
                    var filePath = Path.Combine(Startup._appEnvironment.ApplicationBasePath, "Images", $"images-{language}.txt");
                ...
                {
                    await TraceInfo.Tracer.Exception("ImageRepository:Get", ex);
                }
                return results;
