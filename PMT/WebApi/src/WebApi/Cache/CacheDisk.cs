...
namespace WebApi.Cache
{
    ...
    {
        private static bool _enableDebugTrace;
        ...
        {
            return Task.Run(() => {
                _enableDebugTrace = Startup.AppSettings.CacheSettings.Disk.EnableTrace;
                _maxSizeInMb = Startup.AppSettings.CacheSettings.Disk.MaxSizeInMb;
                _directory = Path.Combine(Startup._appEnvironment.ApplicationBasePath, "DataSource", "cache");
        ...
        public static Task Set(string key, object value, DateTime? expiresAt)
        {
            return Task.Run(async () => {
                ...
                {
                    await Tracer.Error("CacheDisk:Set", ex.ToString());
                }
            });
        ...
        public static Task<string> Get(string key)
        {
            return Task.Run(async () =>  {
                ...
                {
                    await Tracer.Error("Cache:Get", ex.ToString());
                    value = string.Empty;
                }
